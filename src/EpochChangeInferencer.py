...
class EpochChangeInferencer(tf.keras.callbacks.Callback):
  ...
  def __init__(self, flexmodel, config_file):
    ...
    self.image_files += glob.glob(self.images_dir + "/*.npy")
    # 2025/09/14
  ...
  def on_epoch_end(self, epoch, logs):
    for image_file in self.image_files:
      ...
      # 2026/09/05 Added the following few lines for NPY image files.
      if basename.endswith(".npy"):
         data = np.load(image_file)
  
         predicted = self.flexmodel.predict(data)
         basename = basename.replace(".npy", ".png")
         filename = "Epoch_" +str(epoch+1) +  basename
